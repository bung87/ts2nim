...
function convertLogicalExpression(expression: any): string {
  ...
  result = `${tsType2nimType(expression.left)} ${op} ${tsType2nimType(
    expression.right
  )}`;
  return result;
}
  ...
  switch (typ) {
    ...
    case parser.AST_NODE_TYPES.MemberExpression:
      if (typeAnnotation.property === 'length') {
        ...
        if (typeAnnotation.computed) {
          result = `${tsType2nimType(typeAnnotation.object)}[${tsType2nimType(
    ...
    case parser.AST_NODE_TYPES.ExpressionStatement:
      {
        ...
        switch (typeAnnotation.expression.type) {
          ...
          default:
            ...
            console.log(
              'tsType2nimType:ExpressionStatement',
    ...
    case parser.AST_NODE_TYPES.ReturnStatement:
      switch (typeAnnotation.argument.type) {
        ...
        default:
          result = getLine(
            'return ' + tsType2nimType(typeAnnotation.argument, indentLevel),
            indentLevel + 1
          );
    ...
    case parser.AST_NODE_TYPES.LogicalExpression:
      ...
      break;
    case parser.AST_NODE_TYPES.AssignmentExpression:
      result = `${tsType2nimType(typeAnnotation.left)} ${
        typeAnnotation.operator
      } ${tsType2nimType(typeAnnotation.right)}`;
      break;
...
function convertConditionalExpression(expression: any): string {
  let result = '';
  result = `if ${tsType2nimType(expression.test)}: ${tsType2nimType(
    expression.consequent
  )} else: ${tsType2nimType(expression.alternate)}`;
  return result;
}
  ...
  if (node.id.type === parser.AST_NODE_TYPES.ObjectPattern) {
    node.id.properties.forEach((prop: any) => {
      ...
      if (
        ...
        result += getIndented(
          `${name} = ${tsType2nimType(prop.value.right)}`,
          1
        );
      }
    });
...
class Transpiler {
  ...
  writeNode(node: any, indentLevel = 0) {
    switch (node.type) {
      ...
      case parser.AST_NODE_TYPES.ForInStatement:
        ...
        const result = `for ${node.left.declarations.map((y: any) =>
          convertVariableDeclarator(y)
        )} in ${rightName}:`;
        this.writeLine(result, indentLevel);
      ...
      case parser.AST_NODE_TYPES.ReturnStatement:
        if (node.argument) {
          if (node.argument.type === parser.AST_NODE_TYPES.CallExpression) {
            ...
            this.writeLine(
              'return ' + tsType2nimType(node.argument),
              indentLevel
            );
  ...
  handleFunction(node: any, pname: string, isExport = true, indentLevel = 0) {
    ...
    const returnType = node.returnType
      ? tsType2nimType(node.returnType.typeAnnotation)
      : 'auto';
